      ...
      {aba === "importar" && (
        <div>
          ...
          <div className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-2xl p-6 max-w-3xl mt-6">
            <h2 className="font-semibold text-sm mb-3">Aguardando confirmação</h2>
            {carregandoImportacoes ? (
              <div className="skeleton h-24 rounded-xl" />
            ) : importacoes.filter((i) => i.status === "pendente").length === 0 ? (
              <p className="text-xs text-[var(--color-text-muted)]">Nada pendente — toda importação já foi revisada.</p>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full text-xs">
                  ...
                  <tbody>
                    {importacoes
                      .filter((imp) => imp.status === "pendente")
                      .map((imp) => {
          ...
          <div className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-2xl p-6 max-w-3xl mt-6">
            ...
            {carregandoImportacoes ? (
              ...
              <p className="text-xs text-[var(--color-text-muted)]">Nenhuma importação já resolvida ainda.</p>
            ) : (
              (() => {
                // Agrupado por dia (importado_em já vem ordenado desc de carregarImportacoes,
                // então o Map preserva a ordem certa sem precisar reordenar).
                const historico = importacoes.filter((i) => i.status !== "pendente");
                const porDia = new Map<string, typeof historico>();
                for (const imp of historico) {
                  const dia = imp.importado_em.slice(0, 10);
                  if (!porDia.has(dia)) porDia.set(dia, []);
                  porDia.get(dia)!.push(imp);
                }
                return (
                  <div className="space-y-2">
                    {Array.from(porDia.entries()).map(([dia, imps]) => {
                      const totalLinhas = imps.reduce((soma, i) => soma + i.qtd_linhas, 0);
                      ...
                      return (
                        <div key={dia} className="border border-[var(--color-border)] rounded-xl overflow-hidden">
                          ...
                          >
                            <span className="font-semibold">{formatarData(dia)}</span>
                            <span className="text-[var(--color-text-muted)]">
                              {imps.length} arquivo(s) · {totalLinhas} linha(s) {expandido ? "▲" : "▼"}
                            </span>
                          </button>
                          {expandido && (
                            <table className="w-full text-xs border-t border-[var(--color-border)]">
                              <thead>
                                <tr className="text-left text-[var(--color-text-muted)]">
                                  ...
                                  <th className="px-3 py-1.5 font-semibold">Conta</th>
                                  <th className="px-3 py-1.5 font-semibold">Importado em</th>
                                  <th className="px-3 py-1.5 font-semibold text-right">Linhas</th>
                                  <th className="px-3 py-1.5 font-semibold text-center">Status</th>
                                </tr>
                              </thead>
                              <tbody>
                                {imps.map((imp) => {
                                  ...
                                  return (
                                    <tr key={imp.id} className="border-t border-[var(--color-border)]">
                                      <td className="px-3 py-1.5 max-w-[220px] truncate" title={imp.nome_arquivo}>{imp.nome_arquivo}</td>
                                      <td className="px-3 py-1.5 whitespace-nowrap">{conta ? nomeConta(conta) : "—"}</td>
                                      <td className="px-3 py-1.5 whitespace-nowrap">{new Date(imp.importado_em).toLocaleString("pt-BR")}</td>
                                      <td className="px-3 py-1.5 text-right">{imp.qtd_linhas}</td>
                                      <td className="px-3 py-1.5 text-center">
                                        ...
                                        >
                                          {imp.status === "confirmada" ? "confirmada" : "descartada"}
                                        </span>
                                      </td>
                                    </tr>
                                  );
                              ...
                              {itensDoDia.map((l) => (
                                <tr key={l.id} className="border-b border-[var(--color-border)] last:border-0 hover:bg-[var(--hover-bg)]">
                                  ...
                                  <td className="px-3 py-2">
                                    ...
                                    ) : ehBoletoSemResolucao(l) ? (
                                      // Sem atalho de "cadastrar conta nova" aqui de propósito: o
                                      // processo é sempre cadastrar a conta a pagar ANTES dela ser
                                      // paga, então um boleto sem nenhuma correspondência é, por si
                                      // só, uma anomalia -- precisa de investigação (conta cadastrada
                                      // com fornecedor/valor diferente? pagamento fora do processo?),
                                      // não de um clique que cria outra conta por cima e esconde o
                                      // problema. Pra cadastrar mesmo assim, é preciso ir na tela de
                                      // Contas a Pagar de propósito, fora do fluxo rápido do extrato.
                                      <div className="flex flex-col gap-1 items-start">
                                        <span className="px-2 py-1 rounded-lg bg-red-50 text-red-700 border border-red-200 text-[10px] font-semibold">
                                          ⚠️ Nenhuma conta a pagar encontrada — confira antes de ignorar
                                        </span>
                                        {podeEscrever && (
                                          <button
                                            type="button"
